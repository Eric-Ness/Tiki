...
If `activeIssue` is null or `status` is "idle":
  - Inform user: "No active issue to ship. Run `/tiki:get-issue` first."
  - Abort

Read `.tiki/plans/issue-<N>.json` for issue details.
...
```json
{
  ...
  "lastCompletedIssue": <shipped issue number>,
  "lastCompletedAt": "<current ISO timestamp>",
  "errorMessage": null,
  "currentPhase": null,
  "totalPhases": null
}
