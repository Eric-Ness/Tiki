...
type: prompt
name: tiki:release
description: Manage releases with subcommands: new, status, add, remove, ship. Group issues into versions and track progress.
allowed-tools: Bash, Read, Write, Glob, Grep, AskUserQuestion, Skill
argument-hint: <subcommand> [args...] (new <version> | status [version] | add <issue> [--to <version>] | remove <issue> | ship <version>)
---
